    ...
    useEffect(() => {
        ...
        if (token) {
            ...
            if (!isNewUser && preAuthPath && !isLoop) {
                ...
                router.push(isNewUser ? `/${locale}/onboarding` : `/${locale}/dashboard/profile`);
            }
        }
    }, [searchParams, router]);
        ...
        <main className="flex min-h-screen flex-col items-center justify-center p-6 bg-background">
            <div className="w-full max-w-md p-8 glass-panel rounded-2xl shadow-2xl relative overflow-hidden">
                ...
                <p className="mt-8 text-center text-gray-500 text-sm">
                    New to Saldaña Music? <Link href={`/${locale}/register`} className="text-primary hover:underline">Apply for Access</Link>
                </p>
            </div>
        </main>
    );
