...
export default function ActionsRow({ sheet }: { sheet: SplitSheetSummary; currentUserId?: string | null }) {
    ...
    const handleDownloadPdf = async () => {
        ...
        try {
            ...
            const res = await fetch(`${API_BASE_URL}/split-sheets/${sheet.id}/pdf`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = `split-sheet-${sheet.id}.pdf`;
            document.body.appendChild(a);
            a.click();
            a.remove();
            window.URL.revokeObjectURL(url);
    ...
    return (
        <div className="flex items-center gap-2">
            <button
                ...
                title={t('System.shareActionTitle')}
            >
                {/* Share Icon */}
            ...
            {(localStatus === 'DRAFT' || !localStatus) && (
                <button
                    ...
                    title={t('System.startSigningTitle')}
                >
                    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {localStatus === 'PENDING_SIGNATURES' && (
                <button
                    ...
                    title={t('System.signAgreementTitle')}
                >
                    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <button
                ...
                disabled={loading}
                className="p-2 bg-neutral-800 rounded-full hover:bg-primary/20 hover:text-primary transition-colors disabled:opacity-50"
                title={t('System.downloadPdfTitle')}
            >
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
