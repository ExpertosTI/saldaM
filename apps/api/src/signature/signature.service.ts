...
type PdfDocFactory = {
  ...
  load: (bytes: Uint8Array) => Promise<PdfDoc>;
};
...
export class SignatureService {
  ...
  private formatPercentage(value: unknown) {
  ...
  async generateSplitSheetPdf(splitSheet: SplitSheet): Promise<Buffer> {
    const pdfDoc = await (PDFDocument as unknown as PdfDocFactory).create();
    const standardFonts = StandardFonts as unknown as StandardFontsLike;
    const rgbFn = rgb as unknown as RgbFn;
    ...
    if (logoPath) {
      try {
        ...
        if (logoBytes.length > 5 * 1024 * 1024) {
          console.warn(
            `Logo too large (${logoBytes.length} bytes), skipping to prevent PDF hang`,
          );
        } else {
        ...
        console.warn(
          'Logo embed failed, continuing without logo:',
    ...
    y -= 22;
    page.drawText(this.safeText(splitSheet.title), {
      x: marginX + 90,
      y,
      size: 14,
      font: fontBold,
      color: rgbFn(0.1, 0.1, 0.1),
    });

    const metaY = height - 55;
    page.drawText(`DATE: ${this.formatDate(splitSheet.createdAt)}`, {
      x: width - marginX - 200,
      y: metaY,
      size: 10,
      font,
      color: rgbFn(0.2, 0.2, 0.2),
    });
    page.drawText(`DOC ID: ${this.safeText(splitSheet.id)}`, {
      x: width - marginX - 200,
      y: metaY - 14,
      size: 8,
      font,
      color: rgbFn(0.35, 0.35, 0.35),
    });
    ...
    const cols = [
      {
        key: 'name',
        w: tableW * 0.42,
        label: 'COMPOSITOR / AUTOR / PUBLISHER',
      },
    ...
    page.drawRectangle({
      ...
      width: tableW,
      height: headerH,
      color: rgbFn(0.97, 0.94, 0.91),
      borderColor: rgbFn(0.85, 0.8, 0.75),
      borderWidth: 1,
    });
    let cx = tableX;
    for (const c of cols) {
      ...
      page.drawLine({
        ...
        thickness: 1,
        color: rgbFn(0.85, 0.8, 0.75),
      });
    }
    y -= headerH;

    const collaborators = splitSheet.collaborators || [];
    for (const collab of collaborators) {
      ...
      const statusColor = collab.hasSigned
        ? rgbFn(0.05, 0.55, 0.2)
        : rgbFn(0.8, 0.0, 0.0);
      ...
      for (const c of cols) {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          const color = c.key === 'sig' ? statusColor : rgbFn(0.1, 0.1, 0.1);
    ...
    page.drawText(`STATUS: ${this.safeText(splitSheet.status)}`, {
      x: marginX,
      y: footerY + 5,
      size: 9,
      font: fontBold,
      color: rgbFn(0.25, 0.25, 0.25),
    });
  ...
  async generateFullSplitSheetPdf(splitSheet: SplitSheet): Promise<Buffer> {
    ...
    page1.drawText('FULL SPLIT-SHEET', {
      x: headerTextX,
      y,
      size: 16,
      font: fontBold,
      color: rgbFn(0.75, 0.55, 0.15),
    });
    y -= 18;
    page1.drawText('DOCUMENTO DE REPARTO DE DERECHOS DE AUTOR', {
      x: headerTextX,
      y,
      size: 10,
      font: fontBold,
      color: rgbFn(0.4, 0.4, 0.4),
    });
    y -= 20;
    ...
    page1.drawText(`Título de la canción: ${this.safeText(splitSheet.title)}`, {
      x: headerTextX,
      y,
      size: 9,
      font,
      color: rgbFn(0.15, 0.15, 0.15),
    });
    y -= 12;
    page1.drawText(
      `Fecha: ${this.formatDate(splitSheet.createdAt)}    Sello: ${this.safeText(splitSheet.label)}    Estudio: ${this.safeText(splitSheet.studio)}`,
      { x: headerTextX, y, size: 9, font, color: rgbFn(0.15, 0.15, 0.15) },
    );
    y -= 14;
    page1.drawLine({
      start: { x: marginX, y },
    ...
    y = drawGridTable(
      page1,
      y,
      'DERECHOS DEL MÁSTER (SOUND RECORDING)',
      masterCols,
      masterRows,
    );

    page1.drawText(`DOC ID: ${this.safeText(splitSheet.id)}`, {
      x: marginX,
      y: 25,
      size: 8,
      font,
      color: rgbFn(0.5, 0.5, 0.5),
    });
    ...
    page2.drawText('DETALLES DE INFORMACIÓN Y DATOS IMPORTANTES', {
      x: marginX,
      y: y2,
      size: 12,
      font: fontBold,
    ...
    const drawLineField = (label: string, value: string) => {
      ...
      page2.drawLine({
        start: { x: marginX + 160, y: y2 - 2 },
        end: { x: width - marginX, y: y2 - 2 },
        thickness: 0.8,
        color: rgbFn(0.75, 0.75, 0.75),
      });
      y2 -= 18;
    };
    ...
    drawLineField(
      ...
      this.formatDate(splitSheet.createdAt),
    );
    ...
    y2 -= 10;
    page2.drawText('DETALLES DE AUTORES Y/O DUEÑOS DE DERECHOS', {
      x: marginX,
      y: y2,
      size: 12,
      font: fontBold,
    ...
    for (let i = 0; i < blocks; i++) {
      ...
      drawLineField(
        'Nombre(s) / Apellido(s):',
        this.safeText(c ? c.legalName || c.email : ''),
      );
      ...
      drawLineField(
        'Sociedad (PRO):',
        this.safeText(c ? c.proAffiliation : ''),
      );
      drawLineField('CAE / IPI:', this.safeText(c ? c.ipi : ''));
      drawLineField(
        'Publishing Company:',
        this.safeText(c ? c.publishingCompany : ''),
      );
      y2 -= 10;
      if (y2 < 120) break;
    }

    page2.drawText(`DOC ID: ${this.safeText(splitSheet.id)}`, {
      x: marginX,
      y: 25,
      size: 8,
      font,
  ...
  async appendAuditTrail(
    originalPdfBuffer: Buffer,
    splitSheet: SplitSheet,
  ): Promise<Buffer> {
    const pdfDoc = await (PDFDocument as unknown as PdfDocFactory).load(
      originalPdfBuffer,
    );
    const standardFonts = StandardFonts as unknown as StandardFontsLike;
    const rgbFn = rgb as unknown as RgbFn;
    const auditPage = pdfDoc.addPage();
    const { height } = auditPage.getSize();
    ...
    // Document ID
    auditPage.drawText(`Document ID: ${splitSheet.id}`, {
      x: 50,
      y,
      size: 10,
      font,
    ...
    y -= 15;
    auditPage.drawText(`Title: ${splitSheet.title}`, {
      x: 50,
      y,
      size: 10,
      font,
    ...
    for (const collab of splitSheet.collaborators) {
      if (collab.hasSigned) {
        auditPage.drawText(`Signed by: ${collab.email}`, {
          x: 50,
          y,
          size: 10,
          font,
        ...
        y -= 12;
        auditPage.drawText(`  Date: ${this.formatDate(collab.signedAt)}`, {
          x: 50,
          y,
          size: 8,
          font,
        ...
        y -= 12;
        auditPage.drawText(`  IP Address: ${collab.ipAddress}`, {
          x: 50,
          y,
          size: 8,
          font,
        ...
        y -= 12;
        auditPage.drawText(`  User Agent: ${collab.userAgent}`, {
          x: 50,
          y,
          size: 8,
          font,
        ...
        if (collab.otpVerified) {
          auditPage.drawText(`  Verification: Email OTP (2FA) Verified`, {
            x: 50,
            y,
            size: 8,
            font,
        ...
        if (collab.signatureHash) {
          auditPage.drawText(`  Agreement Hash: ${collab.signatureHash}`, {
            x: 50,
            y,
            size: 6,
            font,
